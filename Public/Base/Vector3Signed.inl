...
namespace greaper::math
{
	...
	class Vector3Signed
	{
		static_assert(std::is_integral_v<T> && !std::is_unsigned_v<T>, 
			"Vector3Signed can only work with signed intXX types");
	...
	public:
		...
		INLINE constexpr explicit Vector3Signed(const std::array<T, ComponentCount>& arr)
		...
		NODISCARD INLINE constexpr Vector3Signed GetClamped(const Vector3Signed& min,
															const Vector3Signed& max)const noexcept
		{
			return Vector3Signed{
				::Clamp(X, min.X, max.X),
				::Clamp(Y, min.Y, max.Y),
	...
	template<class T> 
	NODISCARD INLINE constexpr Vector3Signed<T> operator+(const Vector3Signed<T>& left,
														const Vector3Signed<T>& right)noexcept
	{
		return Vector3Signed<T>{ left.X + right.X, left.Y + right.Y, left.Z + right.Z };
	}
	template<class T> 
	NODISCARD INLINE constexpr Vector3Signed<T> operator-(const Vector3Signed<T>& left,
														const Vector3Signed<T>& right)noexcept
	...
	template<class T> 
	INLINE Vector3Signed<T>& operator+=(Vector3Signed<T>& left, const Vector3Signed<T>& right)noexcept
	{
	...
	template<class T> 
	NODISCARD INLINE constexpr Vector3Signed<T> operator*(const Vector3Signed<T>& left, T right)noexcept
	{
		return Vector3Signed<T>{ left.X* right, left.Y* right, left.Z* right };
	}
	template<class T> 
	NODISCARD INLINE constexpr Vector3Signed<T> operator/(const Vector3Signed<T>& left, T right)noexcept
	{
	...
	template<class T> 
	NODISCARD INLINE constexpr Vector3Signed<T> operator*(T left, const Vector3Signed<T>& right)noexcept
	{
		return Vector3Signed<T>{ left* right.X, left* right.Y, left* right.Z };
	}
	template<class T> 
	INLINE Vector3Signed<T>& operator*=(Vector3Signed<T>& left, T right)noexcept
	{
	...
	template<class T> 
	NODISCARD INLINE constexpr bool operator==(const Vector3Signed<T>& left, const Vector3Signed<T>& right)noexcept
	{
...
template<>                                                                                                             \
NODISCARD INLINE constexpr greaper::math::Vector3Signed<type> Abs<greaper::math::Vector3Signed<type>>                  \
(const greaper::math::Vector3Signed<type> a)noexcept{ return a.GetAbs(); }                                             \
template<>                                                                                                             \
NODISCARD INLINE constexpr greaper::math::Vector3Signed<type> Clamp<greaper::math::Vector3Signed<type>>                \
(const greaper::math::Vector3Signed<type> a, const greaper::math::Vector3Signed<type> min,                             \
	const greaper::math::Vector3Signed<type> max)noexcept{ return a.GetClamped(min, max); }                            \
template<>                                                                                                             \
NODISCARD INLINE constexpr greaper::math::Vector3Signed<type> Sign<greaper::math::Vector3Signed<type>>                 \
	(const greaper::math::Vector3Signed<type> a)noexcept{ return a.GetSignVector(); }
