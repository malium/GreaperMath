...
namespace greaper::math
{
	...
	public:
		using value_type = T;
		using vector_type = Vector2Real<value_type>;
		...
		INLINE constexpr explicit Matrix2Real(const std::array<T, ComponentCount>& arr)noexcept
			:R0(arr[0], arr[1]), R1(arr[2], arr[3]) {  }
		INLINE constexpr Matrix2Real(const vector_type& r0, const vector_type& r1)noexcept :R0(r0), R1(r1) {  }
		...
		NODISCARD INLINE constexpr bool IsNearlyEqual(const Matrix2Real& other,
		...
		{
			return std::format("{}, {}, {}, {}", R0.X, R0.Y, R1.X, R1.Y);
		}
	...
	template<class T> NODISCARD INLINE constexpr Matrix2Real<T> operator+(const Matrix2Real<T>& left,
																			const Matrix2Real<T>& right)noexcept
	{
		return { left.R0 + right.R0, left.R1 + right.R1 };
	}
	template<class T> NODISCARD INLINE constexpr Matrix2Real<T> operator-(const Matrix2Real<T>& left,
																			const Matrix2Real<T>& right)noexcept
	{
	...
	template<class T> INLINE Matrix2Real<T>& operator*=(Matrix2Real<T>& left, const Matrix2Real<T>& right)noexcept
	{
		left = (left * right);
		return left;
	}
	...
	template<class T> INLINE Vector2Real<T>& operator*=(Vector2Real<T>& left, const Matrix2Real<T>& right)noexcept
	{
		left = (left * right);
		return left;
	}
	template<class T> NODISCARD INLINE constexpr Vector2Real<T> operator*(const Matrix2Real<T>& left,
																			const Vector2Real<T>& right)noexcept
	{
		return {
	...
	template<class T> INLINE Matrix2Real<T> operator+=(Matrix2Real<T>& left, const Matrix2Real<T>& right)noexcept
	{
		left.R0 += right.R0; left.R1 += right.R1;
		return left;
	}
	template<class T> INLINE Matrix2Real<T> operator-=(Matrix2Real<T>& left, const Matrix2Real<T>& right)noexcept
	{
	...
	template<class T> NODISCARD INLINE constexpr Matrix2Real<T> operator*(const Matrix2Real<T>& left, T right)noexcept
	{
		return { left.R0 * right, left.R1 * right };
	}
	template<class T> INLINE Matrix2Real<T> operator*=(Matrix2Real<T>& left, T right)noexcept
	{
		left.R0 *= right; left.R1 *= right;
		return left;
	}
	template<class T> NODISCARD INLINE constexpr Matrix2Real<T> operator*(T left, const Matrix2Real<T>& right)noexcept
	{
		return { right.R0 * left, right.R1 * left };
	}

	template<class T> NODISCARD INLINE constexpr bool operator==(const Matrix2Real<T>& left,
																	const Matrix2Real<T>& right)noexcept
	{
...
ReflectAsContainer(greaper::math::Matrix2f, 	greaper::refl::RTI_Matrix2f);
ReflectAsContainer(greaper::math::Matrix2d, 	greaper::refl::RTI_Matrix2d);
